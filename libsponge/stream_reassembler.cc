...
StreamReassembler::StreamReassembler(const size_t capacity) : buffer_string(""), buffer_check(""), _reassembled_pt(0), _end_pt(capacity-1), _output(capacity), _capacity(capacity) {}
...
//    DUMMY_CODE(data, index, eof);
    ...
    size_t extended_length = index + data.length(), _reassembled_pt + _output.;
...
size_t StreamReassembler::unassembled_bytes() const {
    size_t unreassembled_bytes_cnt=0;
    for (size_t i = _reassembled_pt; i <=_end_pt; i++)
        if(buffer_check[i]=='1')
            unreassembled_bytes_cnt++;
    return unreassembled_bytes_cnt;
}
